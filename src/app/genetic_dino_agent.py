...
class GeneticDinoAgent:
    def __init__(self, weights=None, activation_function="relu", bias=None):
        ...
        self.bias = bias if bias is not None else np.random.uniform(-1, 1, 2).tolist()
    ...
    def get_action(self, game_state):
        Z = np.sum([s * w  for s, w in zip(game_state, self.weights)])
        print(Z)
        if Z > 0:
            return int(ACTION_UP)
        else:
            return int(ACTION_DOWN)
